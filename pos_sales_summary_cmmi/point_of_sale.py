...
class pos_session(osv.osv):
    ...
    def _compute_cash_all(self, cr, uid, ids, fieldnames, args, context=None):
        ...
        for record in self.browse(cr, uid, ids, context=context):
            result[record.id] = {
                ...
                'usd_cash_journal_id' : False,
                'usd_cash_register_id' : False,
                'cash_control' : False,
            }
            for st in record.statement_ids:
                if st.journal_id.cash_control == True:
                    if st.journal_id.currency and st.journal_id.currency.name == 'USD':
                        result[record.id]['cash_control'] = True
                        result[record.id]['usd_cash_journal_id'] = st.journal_id.id
                        result[record.id]['usd_cash_register_id'] = st.id
                    else:
                        result[record.id]['cash_control'] = True
        ...
        for record in self.browse(cr, uid, ids, context=context):
            ...
            if record.usd_cash_register_balance_end_real:
                ...
                debit_line = (0, 0, {
                    ...
                    'account_id': record.usd_cash_journal_id.default_debit_account_id.id,
                    'journal_id': record.usd_cash_journal_id.id,
                    'period_id': period_id,
                    'debit': 0.0,
    ...
    _columns = {
        ...
        'usd_cash_journal_id' : fields.function(_compute_cash_all,
                                            multi='cash',
                                            type='many2one', relation='account.journal',
                                            string='USD Cash Journal', store=True),
        'usd_cash_register_id' : fields.function(_compute_cash_all,
                                             multi='cash',
                                             type='many2one', relation='account.bank.statement',
                                             string='USD Cash Register', store=True),
        'usd_cash_register_difference' : fields.related('usd_cash_register_id', 'difference',
                type='float',
                string='USD Difference',
                help="Difference between the theoretical closing balance and the real closing balance.",
                readonly=True),

        'crc_cash_register_balance_end_real' : fields.related('cash_register_id', 'balance_end_real',
                type='float',
                digits_compute=dp.get_precision('Account'),
                string="CRC Ending Balance",
                help="Total of closing cash control lines.",
                readonly=False),
        'usd_cash_register_balance_end_real' : fields.related('usd_cash_register_id', 'balance_end_real',
                type='float',
                digits_compute=dp.get_precision('Account'),
        ...
        'cash_register_balance_end' : fields.related('cash_register_id', 'balance_end',
                ...
                readonly=True),
        'usd_cash_register_balance_end' : fields.related('usd_cash_register_id', 'balance_end',
                type='float',
                digits_compute=dp.get_precision('Account'),
                string="Theoretical Closing Balance",
                help="Sum of opening balance and transactions.",
