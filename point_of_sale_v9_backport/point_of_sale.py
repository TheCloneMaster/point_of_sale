...
class pos_config(osv.osv):
    ...
    _columns = {
        ...
        'iface_product_summary':   fields.boolean('Include Taxes in Prices', help='The displayed prices will always include all taxes, even if the taxes have been setup differently'),
...
class pos_session(osv.osv):
    ...
    _columns = {
        ...
        'cash_register_credit_card_end_real' : fields.float(string='Total Tarjeta Cred.', digits_compute=dp.get_precision('Account')),
    }
    ...
    def validate_cb(self, cr, uid, ids, context=None):
        """
        Only sets the pos.session to 'opened' (in progress)
        """
        if context is None:
            context = dict()
    ...
    def print_closing(self, cr, uid, ids, context=None):
        if not context:
            context = {}
        if not ids:
            return {}

        url = 'http://181.193.87.186:8069/hw_proxy/print_xml_receipt'
        receiptDate = fields.date.context_today(self, cr, uid, context=context)
        for record in self.browse(cr, uid, ids, context=context):
            data = {
                "jsonrpc": "2.0",
                "params": {"receipt": u'<receipt align="center" font="a" value-thousands-separator="," width="30"><h1>Alias Accesorios</h1><br/><div font="a"><div>Ced.Juridica: 3-101-680968</div><br/>' + \
                                        u'<h1>Cierre de Caja</h1>' + \
                                        u'<div>Fecha:'+receiptDate+u'</div><br/>' + \
                                        u'<div>Sesion:'+record.name+'u</div><br/>' + \
                                        u'<div>Cajero:'+record.user_id.name+u'</div><br/>' + \
                                        u'<line><left>Total Tarjeta:</left><right>'+str(record.cash_register_credit_card_end_real)+u'</right></line><br/>' + \
                                        u'<line><left>Total Efectivo:</left><right>'+str(record.cash_register_balance_end_real)+u'</right></line><br/>' + \
                                        u'<line><left>Fondo de Caja:</left><right>'+str(-record.cash_register_balance_start)+u'</right></line><br/>' + \
                                        u'<line><left>Monto Deposito:</left><right>'+str(record.cash_register_balance_end_real-record.cash_register_balance_start)+u'</right></line><br/>' + \
                                        u'<br/><br/><br/>' + \
                                        u'<div>________________________________</div><br/>' + \
                                        u'<div>Firma</div><br/>' + \
                                        u'</div></receipt>'},
            }
    ...
    def wkf_action_close(self, cr, uid, ids, context=None):
        ...
        #self.print_closing(cr, uid, ids, context)
    ...
    def _confirm_orders(self, cr, uid, ids, context=None):
        ...
        for session in self.browse(cr, uid, ids, context=context):
            ...
            for order in session.order_ids:
                if order.state in ('done','invoiced', 'cancel'):
                    pass
...
class pos_order(osv.osv):
    ...
    def _process_order(self, cr, uid, order, to_invoice=False, context=None):
        ...
        if paymentsList and not float_is_zero(order['amount_return'], self.pool.get('decimal.precision').precision_get(cr, uid, 'Account')):
            ...
            self.add_payment(cr, uid, order_id, {
                ...
                'payment_date': fields.date.context_today(self, cr, uid, context=context),
    ...
    def unlink(self, cr, uid, ids, context=None):
        for rec in self.browse(cr, uid, ids, context=context):
            for rec_statement in rec.statement_ids:
                if (rec_statement.statement_id and rec_statement.statement_id.state == 'confirm') or rec.state in ('done','paid'):
        ...
        return False  ## super(pos_order, self).unlink(cr, uid, ids, context=context)
    ...
    _columns = {
        ...
        'date_order': fields.datetime('Order Date', select=True),
        'date_order_tz': fields.char('Order Date'),
    ...
    _defaults = {
        ...
        'state': 'draft',
        'name': '/', 
        'date_order': lambda *a: time.strftime(tools.DEFAULT_SERVER_DATETIME_FORMAT),
        'nb_print': 0,
        'sequence_number': 1,
    ...
    def create(self, cr, uid, values, context=None):
        ...
        if values.get('date_order'):
            original_date = datetime.strptime(values.get('date_order'),tools.DEFAULT_SERVER_DATETIME_FORMAT)
            values['date_order_tz'] = fields.datetime.context_timestamp(cr, uid, original_date).strftime(tools.DEFAULT_SERVER_DATETIME_FORMAT)
        else:
            original_date = datetime.today()
            values['date_order_tz'] = fields.datetime.context_timestamp(cr, uid, original_date).strftime(tools.DEFAULT_SERVER_DATETIME_FORMAT)
    ...
    def add_payment(self, cr, uid, order_id, data, context=None):
        ...
        args = {
            'amount': data['amount'],
            'date': data.get('payment_date', fields.date.context_today(self, cr, uid, context=context)),
    ...
    def refund(self, cr, uid, ids, context=None):
        ...
        for order in self.browse(cr, uid, ids, context=context):
            if order.session_id.state == 'opened':
                current_session_id = order.session_id.id
            else:
                current_session_ids = self.pool.get('pos.session').search(cr, uid, [
                    ('state', '!=', 'closed'),
                    ('config_id', '=', 1)], context=context)
                ...
                current_session_id = current_session_ids[0]

            clone_id = self.copy(cr, uid, order.id, {
                'note': order.name + ' REFUND',
    ...
    def _create_account_move_line(self, cr, uid, ids, session=None, move_id=None, context=None):
        ...
        for order in self.browse(cr, uid, ids, context=context):
            ...
            def insert_data(data_type, values):
                ...
                if data_type == 'product':
                    ...
                    key = ('COGS', values['partner_id'], values['product_id'], values['analytic_account_id'], values['debit'] > 0)
                elif data_type == 'tax':
            ...
            for line in order.lines:
                ...
                cacc = line.product_id.property_account_expense and line.product_id.property_account_expense.id
                if not cacc:
                    cacc = line.product_id.categ_id.property_account_expense_categ and line.product_id.categ_id.property_account_expense_categ.id
                ...
                insert_data('COGS', {
                    ...
                    'account_id': dacc,
                    'analytic_account_id': self._prepare_analytic_account(cr, uid, line, context=context),
                    'credit': ((cost>0) and cost) or 0.0,
                    'debit': ((cost<0) and -cost) or 0.0,
                    #'tax_code_id': tax_code_id,
                    #'tax_amount': tax_amount,
                    'partner_id': order.partner_id and self.pool.get("res.partner")._find_accounting_partner(order.partner_id).id or False
                })
        ...
        if move_id: #In case no order was changed
            _logger.warning('MAB - MAB - ALL LINES: %s', all_lines)
