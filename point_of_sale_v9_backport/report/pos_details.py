...
class pos_details(report_sxw.rml_parse):
    ...
    def _pos_sales_details(self, form):
        ...
        pos_ids = pos_obj.search(self.cr, self.uid, [('date_order','>=',form['date_start2']),('date_order','<=',form['date_end2']),
                                                     ('user_id','in',user_ids),('state','in',['done','paid','invoiced']),('company_id','=',company_id)])
    ...
    def _get_payments(self, form):
        ...
        pos_ids = pos_order_obj.search(self.cr, self.uid, [('date_order','>=',form['date_start2']),('date_order','<=',form['date_end2']),('state','in',['paid','invoiced','done']),('user_id','in',user_ids), ('company_id', '=', company_id)])
        data=[]
        if pos_ids:
    ...
    def _get_tax_amount(self, form):
        ...
        pos_ids = pos_order_obj.search(self.cr, self.uid, [('date_order','>=',form['date_start2']),('date_order','<=',form['date_end2']),
                                                           ('state','in',['paid','invoiced','done']),('user_id','in',user_ids), ('company_id', '=', company_id)])
