...
class pos_make_payment(osv.osv_memory):
    ...
    def print_report(self, cr, uid, ids, context=None):
        ...
        datas = {'ids' : [active_id]}
        return self.pool['report'].get_action(cr, uid, [], 'point_of_sale.report_receipt', data=datas, context=context)

        if context is None:
            context = {}
        report_obj = self.pool['report']
        posorder_obj = self.pool['pos.order']
        report = report_obj._get_report_from_name(cr, uid, 'point_of_sale.report_receipt_base')
        active_id = context.get('active_id', [])

        #selected_orders = posorder_obj.browse(cr, uid, ids, context=context)
        selected_orders = posorder_obj.browse(cr, uid, [active_id], context=context)

        docargs = {
            'doc_ids': [active_id], #ids,
            'doc_model': report.model,
            'docs': selected_orders,
        }
        return report_obj.render(cr, uid, ids, 'point_of_sale.report_receipt_base', docargs, context=context)
